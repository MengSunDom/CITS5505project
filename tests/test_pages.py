...
def test_income_unauthenticated(client):
    response = client.get('/income', follow_redirects=True)
    assert response.status_code == 200
    assert b'Login' in response.data


def test_insights_unauthenticated(client):
    response = client.get('/insights', follow_redirects=True)
    assert response.status_code == 200
    assert b'Login' in response.data


def test_shared_data_unauthenticated(client):
