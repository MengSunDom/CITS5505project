...
class User(db.Model):
    ...
    shared_expenses = db.relationship('SharedExpense', backref='shared_with', lazy=True)
...
def get_shared_expenses():
    ...
    shared_expenses = SharedExpense.query.filter_by(shared_with_id=session['user']['id']).all()
    expenses = []
